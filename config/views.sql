...
DELIMITER //
create trigger generarFolio 
before insert on reporte for each row
begin 
	DECLARE next_folio INT;
    SET next_folio = (SELECT AUTO_INCREMENT FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='reporte');
	SET NEW.folio_formato = concat(YEAR(now()),"/", next_folio);
END; //
DELIMITER ;